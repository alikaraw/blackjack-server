import Card from "./Client_Card.js";
...
export default class Player {
    constructor(player){
        ...
        this.handMain = [];
        this.handSplit = [];
        this.insurance = player.insurance;
        this.splittedHand = player.splittedHand;
        this.wins = player.wins;
        this.loses = player.loses;

        for(let iCard = 0; iCard < player.handMain.length; iCard++) {
            let oldCard = player.handMain[iCard];
            let newCard = new Card(oldCard.value, oldCard.symbol);
            newCard.setCardUp(oldCard.isUp);
            this.handMain.push(newCard);
        }

        for(let iCard = 0; iCard < player.handSplit.length; iCard++) {
            let oldCard = player.handSplit[iCard];
            let newCard = new Card(oldCard.value, oldCard.symbol);
            newCard.setCardUp(oldCard.isUp);
            this.handSplit.push(newCard);
        }
    }
    ...
    canSplit() {
        return (
            this.canSurrender() && 
            this.handMain[0].value == this.handMain[1].value &&
